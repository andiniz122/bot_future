...
# Chave da API FRED
FRED_API_KEY = "99f9b66eb363e7c2a7fafb21d6514f8f"
...
# NOVO: Estados do Trading Bot (compatível com frontend)
trading_bot_state = {
    "running": False,
    "last_update": None,
    "active_positions": [],
...
def update_angular_analysis(current_data: Dict):
    ...
    try:
        ...
        btc_prices = [point["btc_price"] for point in cache["price_history"][-10:]]
        gold_prices = [point["gold_price"] for point in cache["price_history"][-10:]]
        dxy_prices = [point["dxy_price"] for point in cache["price_history"][-10:]]
            ...
            try:
                async with session.get(btc_ticker_url, headers=headers, timeout=5) as response:
                    ...
                    if ticker_data and len(ticker_data) > 0:
                        ...
                        if sentiment_cache["btc_sentiment"]["buyers"] > 60: base_fgi += 15
                        elif sentiment_cache["btc_sentiment"]["buyers"] < 40: base_fgi -= 15
...
def get_current_data():
    ...
    try:
        ...
        # CORRIGIDO: last_angular_analysis_iso
        last_angular_analysis_iso = cache["last_angular_analysis"] if isinstance(cache["last_angular_analysis"], str) else None
        ...
        return {
            ...
            "analysis": {
                ...
                "last_angular_analysis": last_angular_analysis_iso,
            ...
            "system_health": {
                ...
                "websocket_status": "connected" if sentiment_cache["websocket_connected"] else "disconnected_or_failed_api"
            }
        }
...
async def get_financial_data_by_period(period: str):
    ...
    try:
        ...
        for asset_name, symbol in SYMBOLS.items():
            ...
            try:
                ...
                if not hist.empty:
                    ...
                    prices = hist['Close'].tolist() if 'Close' in hist.columns else []
                    volumes = hist['Volume'].tolist() if 'Volume' in hist.columns else []
                    asset_dates = hist.index.map(lambda x: x.isoformat()).tolist()

                    # For 1d period, try to get intraday data
                    if period == '1d' and len(prices) <= 2:
                        try:
                            hist_intraday = ticker.history(period="1d", interval="1m")
                            if not hist_intraday.empty:
                                hist_intraday.index = hist_intraday.index.tz_localize(None)
                                prices = hist_intraday['Close'].tolist()
                                volumes = hist_intraday['Volume'].tolist()
                                asset_dates = hist_intraday.index.map(lambda x: x.isoformat()).tolist()
                        except Exception as intraday_e:
                            logger.warning(f"⚠️ Intraday data fetch failed for {symbol}: {intraday_e}")
        ...
        # Get common dates
        if not common_dates and data:
            for asset_name in SYMBOLS.keys():
                if data[asset_name]['dates']:
                    common_dates = data[asset_name]['dates']
                    break
        ...
        response = {
            ...
            "assets": {
                asset_name: {
                    "name": asset_name.capitalize(),
                    "symbol": SYMBOLS[asset_name],
                    "price_data": data[asset_name]['price_data'],
                    "volume_data": data[asset_name]['volume_data'],
                    "volume_avg_formatted": data[asset_name]['volume_avg_formatted']
                } for asset_name in SYMBOLS.keys()
            },
            "timestamp": datetime.now().isoformat()
        }

        logger.info(f"✅ Data fetched for {period}: {len(common_dates)} points")
        return response

    except Exception as e:
        logger.error(f"❌ Error fetching financial data for {period}: {e}")
...
def get_market_sentiment():
    ...
    try:
        ...
        response = {
            ...
            "interpretation": {
                "btc_sentiment": {
                    "sentiment": btc_sentiment_interpretation,
                    "strength": btc_strength,
                    "recommendation": btc_recommendation,
                    "confidence": round(abs(btc_buyers_pct - 50) / 50, 2)
                },
                "paxg_sentiment": {
                    "sentiment": paxg_sentiment_interpretation,
                    "strength": paxg_strength,
                    "recommendation": "Monitor for Gold-specific events",
                    "confidence": round(abs(paxg_buyers_pct - 50) / 50, 2)
                }
            }
        }
        return response
    ...
    try:
        ...
        summary = {
            "btc": {
                ...
                "trend_1h": "INCREASING" if len(btc_buyers_values) > 6 and btc_buyers_values[-1] > btc_buyers_values[-7] else "DECREASING" if len(btc_buyers_values) > 6 else "NEUTRAL",
                "trend_6h": "INCREASING" if len(btc_buyers_values) > 36 and btc_buyers_values[-1] > btc_buyers_values[-37] else "DECREASING" if len(btc_buyers_values) > 36 else "NEUTRAL",
            ...
            "paxg": {
                ...
                "trend_1h": "INCREASING" if len(paxg_buyers_values) > 6 and paxg_buyers_values[-1] > paxg_buyers_values[-7] else "DECREASING" if len(paxg_buyers_values) > 6 else "NEUTRAL",
                "trend_6h": "INCREASING" if len(paxg_buyers_values) > 36 and paxg_buyers_values[-1] > paxg_buyers_values[-37] else "DECREASING" if len(paxg_buyers_values) > 36 else "NEUTRAL",
...
def get_trading_bot_status():
    ...
    try:
        ...
        return {
            ...
            "uptime_seconds": (current_time - datetime.fromisoformat(trading_bot_state["last_update"])).total_seconds() if trading_bot_state["last_update"] else 0,
...
def get_active_positions():
    ...
    try:
        ...
        for position in positions:
            ...
            current_price_multiplier = random.uniform(0.98, 1.02)
...
def get_active_signals():
    ...
    try:
        ...
        # Update the cache with valid signals
...
def get_bot_performance():
    ...
    try:
        ...
        roi_percentage = ((performance["current_balance"] - performance["start_balance"]) / performance["start_balance"]) * 100
        ...
        sortino_ratio = random.uniform(1.2, 3.0)
        ...
        return {
            ...
            "performance_periods": {
                "daily": {
                    ...
                    "return_pct": round((performance["daily_pnl"] / performance["current_balance"]) * 100, 2)
                },
                "weekly": {
    ...
    try:
        ...
        return {
            ...
            "system_health": {
                ...
                "websocket_status": "active" if sentiment_cache["websocket_connected"] else "disconnected",
                "fred_api_status": "simulated",
                "gate_io_api_status": "connected" if sentiment_cache["websocket_connected"] else "failed",
                "trading_bot_status": "running" if trading_bot_state["running"] else "stopped"
            },
            "performance_metrics": {
                "active_connections": len(active_sentiment_websocket_connections),
...
def debug_gateio_connection():
    ...
    try:
        ...
        test_endpoints = [
            ...
            {"name": "WebSocket", "endpoint": "ws://sentiment", "status": "connected" if sentiment_cache["websocket_connected"] else "disconnected"}
        ]
        ...
        for test in test_endpoints:
            connection_test["test_results"][test["name"]] = {
                ...
                "status": test["status"],
                "response_time_ms": random.randint(10, 200) if test["status"] == "success" else None,
                "last_success": current_time.isoformat() if test["status"] == "success" else None
            }
        
        # Overall status
        successful_tests = len([t for t in test_endpoints if t["status"] in ["success", "connected"]])
        connection_test["overall_status"] = "healthy" if successful_tests >= 3 else "degraded" if successful_tests >= 2 else "failed"
        ...
        connection_test["current_data"] = {
            ...
            "websocket_connections": len(active_sentiment_websocket_connections)
        }
...
async def startup_event():
    ...
    logger.info("🚀 Starting Trading Dashboard API v6.0")
